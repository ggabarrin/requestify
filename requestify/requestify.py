...
def generate_request_code(request, selected_template):
    ...
    if request.path.index('/') == 0:  # Relative URI
        print(
            '[*] Relative URI detected. Please enter scheme, hostname and port to generate URL')
        host = None
        port = None
        ...
        if host:
            hostname_input = input(
                f'[+] Hostname \'{host}\' detected in \'Host\' header. Use this hostname? [Y/n] ')
            if hostname_input == 'n' or hostname_input == 'no':
                host = None

        if not host:
            host = input('[+] Enter hostname (e.g. www.wikipedia.org): ')

        if port:
            port_input = input(
                f'[+] Port \'{port}\' detected in \'Host\' header. Use this port? [Y/n] ')
            if port_input == 'n' or port_input == 'no':
                port = None

        if not port:
            port = int(input('[+] Enter port (e.g. 80, 443, ..): '))
...
def main():
    ...
    if not lang:
        ...
        print(
            '[*] Invalid language option. Please select one of the available languages.')
